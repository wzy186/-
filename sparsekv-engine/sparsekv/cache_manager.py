...
class FP8KVCacheManager:
    ...
    def quantize_and_store(
        ...
        k_max = k.abs().amax(dim=(1, 2), keepdim=True)
...
class SparseKVCacheManager:
    ...
    维护每个 head 的 Top-K 重要 KV 索引，非重要 KV 可以丢弃或压缩。
    """
    ...
    def _refresh_topk(self):
        ...
        q_proxy = self.k_cache[:, self.cache_len - 1, :]
    ...
    def get_sparse_kv(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """获取稀疏化后的 K, V 和索引"""
        if self.cache_len == 0:
            empty = torch.empty(0, self.head_dim, dtype=torch.float16, device=self.device)
            return empty, empty, torch.empty(0, dtype=torch.int32, device=self.device)
